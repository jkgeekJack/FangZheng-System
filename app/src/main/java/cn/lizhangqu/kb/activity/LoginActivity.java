...
//		String name=util.getKeyData("username");
//		String pwd=util.getKeyData("password");
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try{
					...
					HttpGet httpGet=new HttpGet(HttpUtil.URL_BASE);
...
//					Log.e("body", total);
					viewstate=total.split(" name=\"__VIEWSTATE\" value=\"")[1].split("\"")[0];
	...
	private void jump2Main() {
		SharedPreferenceUtil util=new SharedPreferenceUtil(getApplicationContext(), "accountInfo");
		SharedPreferenceUtil utilkb = new SharedPreferenceUtil(
				getApplicationContext(), "flag");
		utilkb.setKeyData(LinkUtil.XSGRKB, "FALSE");
		if (!(HttpUtil.txtUserName.equals(util.getKeyData("username")))){
			utilkb.setKeyData(LinkUtil.XSGRKB, "FALSE");
		}
		util.setKeyData("username", HttpUtil.txtUserName);
		util.setKeyData("password", HttpUtil.TextBox2);
	...
	private void login() {
		...
		HttpUtil.__VIEWSTATE=viewstate;
