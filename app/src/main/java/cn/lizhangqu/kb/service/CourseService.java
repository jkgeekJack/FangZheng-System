...
//				if(column.attr("rowspan").equals("3")){
//					column.attr("rowspan","2");
//				}
				int week = fillMap(column, map, i);
				//填充map，获取周几，第几节至第几节
				//作用：弥补不能获取这些数据的格式
				Log.e("几节课",column.attr("rowspan"));
				//将三节课缩短为两节
//				if(column.attr("rowspan").equals("3")){
//					if (column.hasAttr("rowspan")) {
//						try {
//							Log.e("i1",i+"");
//							System.out.println("周"+ week+ " 第"+ (i + 1)+ "节-第"+ (i + Integer.parseInt("2")) + "节");
//							splitCourse(column.html(), startYear,endYear,semester,week, i + 1,i + Integer.parseInt("2"));
